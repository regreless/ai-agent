...
def load_rag_prompts():
    try:
        ...
        logger.error(f"[rag_summarize_prompt_path]在yaml配置项中没有rag_summarize_prompt_path配置项")
        raise e

    try:
        return open(system_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[rag_summarize_prompt_path]解析系统提示词出错，{str(e)}")
        raise e
...
def load_report_prompts():
    ...
    try:
        ...
        logger.error(f"[report_prompt_path]解析系统提示词出错，{str(e)}")
        raise e